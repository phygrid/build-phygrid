...
TODO 2025-07-27: Restructure this to work with Device Setup article

> **Note:** This step requires a freshly provisioned physical PhyOS device.
> If you don't have a physical device available, skip to the [**Connect a PhyVM**](#-connect-a-phyvm) section.
...
For additional details on creating installations and connecting devices, check out this [comprehensive guide](https://learn.phygrid.com/installations/connect-a-new-device-to-an-installation/).
...
Once the VM is running, follow the same steps as in the physical device section to connect it to your installation and verify the deployment.
...
Now let's deploy your code to the connected VM in development mode. Please note that this feature is intended for development and iteration, not for managing production devices.
...
To enable development mode on the PhyOS VM, please follow this guide: [Enabling Developer Mode](/how-to/advanced-phyos-features/#-enabling-developer-mode).
...
**Note:** You can also enable developer mode on a physical PhyOS device using the same steps. Keep in mind that `phy app deploy` requires both the device and your host machine to be discoverable on the same network. 

To verify that your changes were deployed successfully, access your PhyOS shell:
