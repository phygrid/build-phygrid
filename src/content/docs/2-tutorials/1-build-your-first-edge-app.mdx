...
Edge apps are a type of application that runs on PhyStack-connected devices without a graphical user interface. These apps execute locally in the field as fully managed PhyStack containers, providing compute power and logic right where it's needed — at the edge.

Edge apps run on PhyOS-enabled devices. Just like screen apps, edge apps are:

- Published and managed via your PhyStack tenant
...
Whether you're building hardware integrations, deploying AI workloads, or processing data locally, edge apps give you full control at the device level — all while being securely managed by the PhyStack platform.

## 🧰 Prerequisites

Official PhyStack templates and examples use Node with TypeScript, offering modern tooling, strong typing, and native SDK support. This is the stack we'll be working with throughout this tutorial.
...
Ensure you have Docker Desktop installed and configured for PhyStack.
...
4. Create and upload the app to your PhyStack tenant

Once complete, your app will be available in the PhyStack Console, ready to be installed and deployed to any edge capable device. We will cover this part later in the tutorial.
...
- **package.json** containing useful scripts and Docker configuration in the `docker-credentials` property
...
### 📝 Docker Configuration in package.json

Your edge app's `package.json` includes a `docker-credentials` property that contains all Docker-related configuration:
...
```json
{
  ...
  "docker-credentials": {
    "registry": "docker.io",
    "image": "docker.io/myusername/my-edge-app",
...
#### docker-credentials Properties

- **registry** - The container registry URL (e.g., `"docker.io"`, `"ghcr.io"`)
- **image** - The full image name including registry and namespace (e.g., `"docker.io/myorg/myapp"`)
- **architectures** - Array of target CPU architectures for your edge app:
  - **Single architecture:** `["amd64"]` or `["arm64"]`
  - **Multiple architectures:** `["amd64", "arm64"]`
- **isPrivate** - Boolean indicating whether the Docker repository is private (`true`) or public (`false`)
- **pull** - Read-only credentials used by devices to pull the container image:
  - **username** - Docker registry username
  - **password** - Read-only access token or password

> **Security Note:** The `pull` credentials should have read-only access. Never use credentials with push/write access in this property, as they are deployed to devices.

When you run `yarn build`, the CLI will automatically build Docker images for all architectures specified in the `architectures` array. This ensures your app can be deployed to different device types without rebuilding.
