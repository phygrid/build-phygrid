...
Screen Apps are ideal for creating interactive user experiences and visual interfaces that connect to the PhyStack platform. Common applications include:
...
- **Deployment:** Hosted and delivered by PhyStack's app hosting service
...
Apps are tightly integrated into the PhyStack ecosystem, meaning:
...
Official PhyStack templates and examples use React with TypeScript, offering modern tooling, strong typing, and native SDK support. This is the stack we'll be working with throughout this tutorial.
...
Once complete, your app will be available in the PhyStack Console, ready to be installed and deployed to any capable device. We will cover this part later in the tutorial.
...
The `app.tsx` file demonstrates the usage of `@phystack/hub-client` package for platform integration.
...
- **Device deployment**: Connects to the PhyStack platform to receive real settings and establish communication 
...
Browser devices provide a convenient way to test your screen app directly on your computer in an environment that simulates a real PhyStack device. This option is good for development and testing, but not intended for production deployments.
...
You should now see the newly added "Hello PhyStack!" heading and the updated product name from your settings on both the device screen and the connected browser window.
