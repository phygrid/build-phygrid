...
2. **Environment ID**  
   Create this in the Grid Console under **<Terminal /> Operations &rarr; <Cloud /> Environments**. Examples might include `dev` or `staging`.

3. **Access Token**  
   Generate an access token in the **<SquaresFour />Apps &rarr; <Terminal />Developers** tab. You’ll need this for all API requests: [Products API](/grid-products/api?id=request-authentication), [Spaces API](/grid-products/spaces-api?id=request-authentication)
