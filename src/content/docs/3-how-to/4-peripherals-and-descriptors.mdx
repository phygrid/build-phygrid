...
**Peripherals** are a concept in the PhyStack platform for connecting and managing external hardware devices that are not directly integrated into the platform itself. Examples include cameras, card readers, barcode scanners, and printers.
...
Peripherals available in your tenant can be discovered, monitored, and configured via the Console, just like other devices or apps — significantly improving observability and maintainability in live physical environments.

## 🔄 Peripheral Twin Lifecycle
...
Thanks to Phyhub's smart routing, messages between twins are delivered either locally (on the same device) or via the cloud, depending on the setup. This enables **cross-device twin subscriptions**—any device in your tenant can subscribe to any peripheral twin, making it easy to build complex, connected and modular solutions.

## 💪 Peripheral Flexibility

Peripherals often represent **physical hardware devices**—such as scanners, sensors, or access controllers.
...
This flexibility allows developers to standardize interactions and status reporting for both tangible and intangible parts of their system—using the same twin and descriptor patterns throughout.
...
- **Actions** — commands the twin can receive and respond to
- **Desired Properties** — configuration values the twin should follow, configurable in Console
- **Reported Properties** — the current state of the twin, reported from the device to the platform
- **Events** — structured outputs that other components can subscribe to
...
4. The screen app receives events and reacts to door status changes—without needing any knowledge of the underlying hardware or control logic.

5. Importantly, the screen app does not need to run on the same device—thanks to Phyhub's routing, any subscribed device in your tenant can react to those events.
...
#### ✅ A PhyStack Tenant
You'll need access to a PhyStack tenant. If you don't yet have one, please refer to the [Tenant Setup](/getting-started/tenant-setup/) guide.
...
You'll find that `schema.ts` exports an interface that defines your descriptor's actions, events, desired properties, and reported properties — all of which we mentioned earlier.
...
The PhyStack Console uses both files to render dynamic configuration forms with custom components and widgets.
...
**Desired Properties** — This part defines configuration settings that can be modified through the Console interface on twin detail pages or in your app's code during runtime.
...
**Reported Properties** — Defines the structure of status data that your twin will report back to the platform. This validates property updates from your app and enables the Console to display twin status information on detail pages.

**Actions** — Specifies the commands that other apps can invoke on this twin. This establishes a clear contract for inter-app communication, defining both available actions and their expected parameters. For example, a printer descriptor might define a `print` action that accepts document content as a parameter.

**Events** — Defines the notifications that this twin can broadcast to subscribers. Apps listening to these events know exactly what data to expect when reacting to twin state changes. For example, a barcode scanner descriptor could emit a `scan` event containing the scanned code data.

### 📤 Publish Your Descriptor
...
1. **Retrieves existing peripheral twins** for this app instance using the PhyHub client (part of PhyStack SDKs). An app instance represents an installation deployed to a specific device.
...
**Peripheral Name** — A human-readable label used to easily identify the peripheral, particularly in the PhyStack Console interface.

**Peripheral Hardware ID** — Serves as the unique identifier for the peripheral. In real-world scenarios, this would typically be a serial number, MAC address, or other unique device identifier. Each hardware ID must be unique within a single app instance installation. 

**Desired Properties Configuration** — Optional configuration values that can be passed during twin creation. In our example, we passed a value for the optional `color` desired property defined in our descriptor schema.
...
**Descriptor Association** — Specifies which descriptors define the twin's capabilities, using the format `{ <tenant-slug>/<descriptor-name>: <semver> }`. 
...
Once you've successfully deployed the latest code to your device and confirmed it's running the new version, navigate to your tenant in the PhyStack Console.

From the sidebar menu, navigate to **Operations → Peripherals**.
...
- **Twin Overview** — Basic information about the twin, including ID, descriptors, and device reference
- **Twin Configuration** — A dynamic form based on the peripheral type and associated descriptors
- **Raw Properties View** — Direct access to twin properties for debugging and testing
...
- **Configuration Value** — Pre-populated with the default value from your schema
- **Color** — Displays as a color picker with the value you provided during twin creation
...
setInterval(() => {
  peripheralInstance.emit('exampleEvent', {
    statusText: ['🚀 To the moon!', '🍕 Pizza time!', '🎉 Party on!', '🤖 Beep boop!', '🌈 Chasing rainbows'][Math.floor(Math.random() * 5)],
...
1. **Event Emission** — The peripheral periodically emits `exampleEvent` with random status text and timestamps, showing how peripherals can broadcast information to subscribers
2. **Action Handling** — The peripheral listens for `exampleAction` commands and logs them, demonstrating how other apps can invoke actions on this peripheral
...
```typescript
import { connectPhyClient, PhyHubClient, PeripheralInstance } from '@phystack/hub-client';
...
- **Event Subscription** — Listens for `exampleEvent` from the peripheral and updates the UI with status text. In real-world scenarios, these would typically be hardware-related events such as face detection from a camera or scan completion from a barcode reader
- **Action Invocation** — Sends `exampleAction` to the peripheral when the button is clicked. In real use cases, this could be commands like triggering a printer to print a receipt or instructing a door controller to unlock
...
- **Real-time status updates** — The status text changes every few seconds as your edge app emits peripheral events with random payloads
- **Action button** — A "Send Peripheral Action" button that triggers actions on the peripheral
