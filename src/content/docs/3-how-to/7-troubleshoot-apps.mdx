...
>
> ```shell
> uploadScreenshot(): Upload block blob xxxxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxxx.jpg successfully xxxxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxxx
...
| Issue                        | Symptoms                      | Solution                                        |
| ---------------------------- | ----------------------------- | ----------------------------------------------- |
| **Service Not Starting**     | Service shows "failed" status | Check system resources and service dependencies |
| **Display Not Initializing** | Stuck at logo screen          | Verify HDMI connections and display settings    |
| **App Loading Failures**     | Blank screen after logo       | Check app settings                              |
| **Configuration Errors**     | Service fails to start        | Verify screen twin configuration and settings   |
...
>   "twinId": "************************",
>   "data": {
>     "screenshot": "https://************************.blob.core.windows.net/screenshots/****************-****-****-****-************.jpg",
>     "appStatus": "running",
...
Screen apps can generate various events that provide insights into user interactions, app performance, and operational status. To view the events generated by your screen application, including reported properties, twin messaging, and updates received by the app, run the following command:
...
#### **Troubleshoot Screen App Signals**
...
> **Expected output:** You should see signal generation activities from the screen app:
>
> ```shell
> sendEventSignal(): sending Event Signal from device ****************-****-****-****-************ {
>   deviceId: '****************-****-****-****-************',
>   data: {
>     g: 'UAE',
>     b: '****************-****-****-****-************',
...
- **Signal Generation:** Verify that screen apps are generating appropriate signals for user interactions
- **Signal Types:** Confirm that Client, Session, and Event signals are being sent as expected
- **Data Accuracy:** Ensure signal payloads contain correct screen app context and user data
- **Timing Patterns:** Check that signals are being sent at appropriate intervals during app usage
- **Error Tracking:** Look for signals related to screen app errors or performance issues

**Common screen app event and signal issues and solutions:**

| Issue                         | Symptoms                           | Solution                                                |
| ----------------------------- | ---------------------------------- | ------------------------------------------------------- |
| **Events Not Received**       | No event logs, missing data        | Check network connectivity and PhyHub connection        |
| **Event Processing Failures** | Error messages in logs             | Verify event handler implementation and data validation |
| **Signal Generation Issues**  | Missing user interaction signals   | Check signal implementation in screen app code          |
| **Property Reporting Issues** | Properties not updating in console | Check property reporting frequency and data format      |
| **Display Event Problems**    | Missing display-related events     | Verify display event monitoring and reporting           |
...
| ------------------------- | ------------------------- | ----------------------------------------------------- |
| **Port Forwarding Fails** | Connection refused errors | Check device connectivity and firewall settings       |
| **DevTools Not Loading**  | Blank or error pages      | Verify port forwarding and Chrome configuration       |
| **Console Not Updating**  | Missing or stale logs     | Refresh the DevTools connection or restart forwarding |
| **Network Tab Empty**     | No request data visible   | Check if app is making HTTP requests and refresh      |
| **Performance Issues**    | Slow DevTools response    | Close unnecessary tabs and optimize network settings  |
...
- **Available space** should be sufficient for app operations (recommended >20% free)
- **High usage** (>80%) may indicate the need for cleanup or storage expansion
- **Critical levels** (<10% free) can cause system instability and app failures
- **Temporary filesystems** (tmpfs) should show minimal usage

✅ **Checkpoint:** You now have a comprehensive troubleshooting reference for both Edge and Screen Apps deployed on the Phygrid platform. This guide provides you with:
