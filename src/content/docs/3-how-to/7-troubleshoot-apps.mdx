...
Monitor the connection attempts and status:
...
> When device establish connection with PhyHub successfully, you can expect logs similar to following
>
> ````shell 2025-06-20T07:45:17.857Z [INFO] [phylogger-device-phyos]
> connectPhyHub(): Connected to PhyHub 2025-06-20T07:45:17.863Z [INFO]
> [phylogger-device-phyos] connectPhyHub(): Device connected with
> details: { deviceId: 'xxxx-xxxx-xxxxx-xxxx-' }
> 2025-06-20T07:45:17.863Z [INFO] [phylogger-device-phyos]
> connectPhyHub(): Environment already provisioned with device
> credentials, skipping configuration 2025-06-20T07:45:17.863Z [INFO]
> [phylogger-device-phyos] connectPhyHub(): Caching twins from server
> response... ```
> ````

If PhyHub connection is failing, verify network reachability.

```shell
curl http://portal-<region>.omborigrid.com/
```

> **Note:** Replace <region> by your device's region (see step 2)

**What to look for:**

- Ensure device is able to connect to phyhub
- Ensure device is able to cURL `http://portal-<region>.omborigrid.com/`
- If no response is received, there's likely a network, ensure your network is not blocking `http://portal-<region>.omborigrid.com/`
...
Ensure the device is receiving configuration updates from PhyHub:
...
- Whether the device is successfully receiving twin configurations
- If synchronization is occurring at expected intervals
- Any errors during the twin initialization process

### **Signal Transmission Issues**

- **Symptoms:** Events not appearing in dashboard, analytics data missing
- **Quick Check:** Verify signal generation and network connectivity
- **Solution:** See [Troubleshoot Signals](#7-troubleshoot-signals) section
...
Device connectivity issues are among the most common problems encountered when deploying Edge Apps. When a device cannot establish or maintain a connection with PhyHub, it prevents app deployment, configuration updates, and real-time monitoring. This section provides comprehensive diagnostic procedures to identify and resolve connectivity problems.

#### **Step 2: Check PhyHub Connection Status**
...
- Replace `<region>` with your device's region from Step 1
- If no response is received, there's likely a network or proxy configuration issue
...
| Issue                    | Symptoms                                   | Solution                                     |
| ------------------------ | ------------------------------------------ | -------------------------------------------- |
| **Network Timeout**      | Connection attempts timeout                | Check network stability and firewall rules   |
| **Proxy Authentication** | 407/401 errors in logs                     | Verify proxy credentials and settings        |
| **DNS Resolution**       | Cannot resolve portal URLs                 | Check DNS configuration and network settings |
| **Certificate Issues**   | SSL/TLS handshake failures                 | Verify system time and certificate validity  |
| **Rate Limiting**        | Connection refused after multiple attempts | Check for excessive connection attempts      |
...
Refer to the Signal Key Lookup Table at the end for decoding short keys.
...
## **📦 Signal Key Mapping Table**

Use this table to decode compressed signal payloads.
