...
- **Single Source of Truth:** Using TypeScript ensures the same definitions are used
  throughout the codebase—both in the logic and in schema generation.
...
- 🔄 **Single Source of Truth:** The TypeScript interface serves
  both the developer and the platform that renders the UI.
- ✅ **Built-in Validation:** UI components and backend validation share the same rules,
  reducing bugs and mismatches.
- 🛠️ **Tooling Friendly:** Changes in the schema are instantly reflected in the generated forms.
...
**1️⃣ Create a New Edge App**

- Run the command: `phy app create`
- Enter the desired app name.
- When prompted, select:
  ❯ Edge Application (Node.js)
...
The Product Picker allows users to select products from their Grid catalog,
enabling product-specific configurations and data management. This is essential for
retail applications, product displays, and e-commerce integrations that need to
reference specific products from the Grid platform.
...
**Key Features:**
- The `productPicker` UI component presents available products from your Grid catalog
- Users can select a specific product to enable product-specific functionality
- The selected product includes essential details like ID, image, and source URL
- Product information can be used to display product details or fetch real-time data
