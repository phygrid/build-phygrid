...
Here's we will updating the `schema.ts` file:
...
This example works PhyOS devices (PhyBrowsers, PhyVM, PhyOS Devices) and locally:
...
1. **Update app settings in `src/settings/index.json`**:
...
```
2. Run this command `yarn run start`

Now you can develop and you can see the live updates locally


**To run this app on PhyOS devices (PhyBrowsers, PhyVM, PhyOS Devices) , follow these steps:**
...
#### 🌐 Edge App Example
Now we can apply the same steps on the edge apps
- Run the command: `phy app create`
- Enter the desired app name.
- When prompted, select:
