...
Access comprehensive analytics aggregated across all devices, installations, and spaces within your entire tenant, providing organization-wide insights that enable executive-level decision making, strategic planning, and comprehensive performance monitoring across your entire Phygrid ecosystem.
...
All new Screen Apps and Edge Apps include a basic analytics schema located in `src/analytics-schema.ts`. The default configuration provides session tracking:
...
This section demonstrates how to implement analytics for events using a product rating system as an example. You'll learn how to create user interface elements, emit events, and configure analytics to display the tracked data.
...
## 🛠️ Advanced Example: Real-time Events Monitoring

This advanced example demonstrates how to implement real-time events analytics by creating a dedicated analytics tab for monitoring events. You'll learn how to emit monitoring events, configure real-time status cards, and organize analytics into multiple tabs for better data organization.

**Key Requirements for Monitoring Events:**

- **Event Naming Convention:** All real-time monitoring events must start with `MONITOR_` prefix (e.g., `MONITOR_STATUS`, `MONITOR_HEALTH`)
- **Real-time Processing:** Monitoring events are processed immediately for live status updates
- **Status Tracking:** Events can track various states using integer or string fields
- **Visual Indicators:** Configure color-coded status representations for instant recognition

#### **Step 1: Add Status Reporting Interface**

Extend your existing Screen App interface by adding status reporting buttons. These buttons will allow users to manually report system status changes:

```tsx
<StyledButton onClick={() => handleReportStatus(1)}>
  Report Online
</StyledButton>

<StyledButton onClick={() => handleReportStatus(0)}>
  Report Offline
</StyledButton>
```
...
- **Manual Status Control:** Users can manually trigger status updates
- **Visual Feedback:** Clear labeling indicates the status being reported
- **Immediate Response:** Status changes are reflected in real-time dashboards

#### **Step 2: Implement Status Event Handler**

Create a comprehensive event handler that emits properly formatted monitoring events with all required metadata:

```ts
const handleReportStatus = useCallback(
  async (status: 0 | 1) => {
    const { signals } = state
    const instanceProps = await signals.getInstanceProps()
...
- **Session Context:** Includes session ID for proper event attribution
- **Data Residency:** Ensures data compliance with regional requirements
- **Timestamp Precision:** Uses ISO format for accurate time tracking
- **Status Storage:** Uses `int1` field to store status values (0=Offline, 1=Online)
- **Organizational Context:** Includes space and tenant IDs for proper aggregation

#### **Step 3: Configure Multi-Tab Analytics Schema**

Update your analytics schema to include a dedicated monitoring tab. This creates a separate analytics section specifically for real-time status monitoring:
      ...
      cards: [
        {
          type: CardType.MonitoringRealtimeStatus,
          eventType: "MONITOR_STATUS",
          distinctColumns: ["deviceId", "int1"],
          title: "Monitor Status",
          statusReference: {
            1: { label: "Online", color: "#22c55e" },
            0: { label: "Offline", color: "#ef4444" },
          },
        },
      ],
    },
...
**Schema Configuration Features:**

- **Tab Organization:** Creates separate "Monitor Status" tab for monitoring analytics
- **Real-time Visualization:** Uses `MonitoringRealtimeStatus` card for live updates
- **Device Tracking:** Monitors status per device using `distinctColumns`
- **Status Mapping:** Color-coded status indicators for visual clarity
- **Custom Labels:** Meaningful status descriptions for better user understanding

#### **Step 4: Deploy and Activate Monitoring**

Complete the deployment process to activate your real-time monitoring dashboard:
...
3. **Update Installation Configuration:**
   ...
   - Navigate to installation settings in the Phygrid Console
   - Update the app version to the newly published version
   - Click "Save All Changes" to apply the configuration

4. **Verify Real-time Monitoring:**
   - Access your Screen App on the device
   - Click the status reporting buttons to generate events
     ![Status reporting buttons](../../../images/3-reports/screen-app-report-status.png)
   - Monitor the new "Monitor Status" tab in the analytics dashboard

![Monitor Status Tab](../../../images/3-reports/screen-app-monitor-tab.png)

**Expected Results:**

- **New Analytics Tab:** "Monitor Status" tab appears in the analytics interface
- **Real-time Updates:** Status changes are reflected immediately in the dashboard
- **Visual Status Indicators:** Color-coded status display provides instant status recognition
- **Device-level Tracking:** Individual device status is monitored and displayed
- **Historical Context:** Status changes are tracked over time for trend analysis

**Use Cases for Real-time Monitoring:**

- **System Health Monitoring:** Track operational status of devices and applications
- **Equipment Status Tracking:** Monitor machinery, sensors, or other connected devices
- **Service Availability:** Track uptime and availability of critical services
- **Alert Generation:** Identify when systems require attention or maintenance
- **Performance Optimization:** Analyze status patterns to improve system reliability

## 📅 Filter Analytics by Date

All analytics dashboards in the Phygrid platform include powerful date filtering capabilities that allow you to analyze data across specific time periods. The date range selector provides flexible options to view historical trends, compare performance across different periods, and focus on specific timeframes for detailed analysis.
