...
- Ombori CLI - [Check installation guide](../../phygrid-cli/cli-setup/)
- Git
- Docker

Deploying modules to the Ombori Grid requires a **Docker Image Registry**. We recommmend signing up for the [Azure Container Registry](https://azure.microsoft.com/en-us/services/container-registry/).
...
To get the slug of your organisation, run `omg org list` command ([Reference](/phygrid-cli/cli-reference/#list-organizations)).
...
<InfoBox>Read more about [Settings Schema](/general/schema.md)</InfoBox>
...
![Your Apps](./images/marketplace-your-apps.png)

Then next step, find the IoT App you just uploaded, and press `Install`.

![IOT App](./images/marketplace-iot-app.png)
...
<InfoBox>
  Replace `[device-name]` with the name of the device you want to direct-deploy
...
To understand what the different `yarn` commands do we've used, you can check `package.json` to see what the commands actually triggered. These commands are mostly a short and easy reference to the `phy` CLI, for which any more information you can find in the [CLI Reference](/phygrid-cli/reference).
